...
export function calculateSettlement(
  ...
  return inputs.map(input => {
    ...
    // 엑셀 파일의 고용/산재보험 값 직접 사용
    const baseEmploymentInsurance = excelEmploymentInsurance
    const baseAccidentInsurance   = excelAccidentInsurance
    ...
    // 세금신고금액 = 기본정산금액 + 지사프로모션
    const taxBaseAmount = Math.max(0, baseAmount + promotionAmount)

    // 소득세 = 세금신고금액 × income_tax_rate (원단위 절상)
    const incomeTaxDeduction = Math.ceil(taxBaseAmount * settings.income_tax_rate)
    ...
    // 최종정산금액 = 기본정산금액 - 시간제보험료 - 고용보험 - 산재보험
