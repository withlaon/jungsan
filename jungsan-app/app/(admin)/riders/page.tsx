...
function downloadRiderList(riders: Rider[], label: string) {
  ...
  const header = ['ê°€?…ì¼', '?¼ì´?”ëª…', '?„ì´??, 'ì£¼ë??±ë¡ë²ˆí˜¸', '?°ë½ì²?, '?€?‰ëª…', 'ê³„ì¢Œë²ˆí˜¸', '?ˆê¸ˆì£?, '?íƒœ']
  ...
  XLSX.utils.book_append_sheet(wb, ws, '?¼ì´?”ëª©ë¡?)
  const today = new Date().toISOString().slice(0, 10)
  XLSX.writeFile(wb, `?¼ì´?”ëª©ë¡?${label}_${today}.xlsx`)
}

function downloadSampleExcel() {
  const wb = XLSX.utils.book_new()
  const data = [
    ['ê°€?…ì¼', '?¼ì´?”ëª…*', '?„ì´??, 'ì£¼ë??±ë¡ë²ˆí˜¸', '?°ë½ì²?, '?€?‰ëª…', 'ê³„ì¢Œë²ˆí˜¸', '?ˆê¸ˆì£¼ëª…'],
    ['2026-01-01', '?ê¸¸??, 'rider001', '900101-1234567', '010-1234-5678', 'êµ???€??, '123-456-7890123', '?ê¸¸??],
    ['2026-01-15', 'ê¹€ì² ìˆ˜', 'rider002', '850520-1234567', '010-9876-5432', '? í•œ?€??, '110-123-456789', 'ê¹€ì² ìˆ˜'],
  ]
  ...
  XLSX.utils.book_append_sheet(wb, ws, '?¼ì´?”ëª©ë¡?)
  XLSX.writeFile(wb, '?¼ì´???€?‰ë“±ë¡??‘ì‹.xlsx')
}
...
export default function RidersPage() {
  ...
  const checkUsernamedup = (val: string, currentId?: string) => {
    ...
    setUsernameError(dup ? '?´ë? ?¬ìš© ì¤‘ì¸ ?„ì´?”ì…?ˆë‹¤.' : '')
  }

  const handleSave = async () => {
    if (!form.name.trim()) { toast.error('?¼ì´?”ëª…???…ë ¥?´ì£¼?¸ìš”.'); return }
    ...
    if (form.rider_username.trim()) {
      ...
      if (dup) { setUsernameError('?´ë? ?¬ìš© ì¤‘ì¸ ?„ì´?”ì…?ˆë‹¤.'); toast.error('?´ë? ?¬ìš© ì¤‘ì¸ ?„ì´?”ì…?ˆë‹¤.'); return }
    }
    if (usernameError) { toast.error('?„ì´??ì¤‘ë³µ???•ì¸?´ì£¼?¸ìš”.'); return }
    ...
    try {
      ...
      if (!res.ok) { toast.error(data?.error ?? '?€???¤íŒ¨'); return }

      toast.success(editingRider ? '?¼ì´???•ë³´ê°€ ?˜ì •?˜ì—ˆ?µë‹ˆ??' : '?¼ì´?”ê? ?±ë¡?˜ì—ˆ?µë‹ˆ??')
      setDialogOpen(false)
      setSearch('')
      refreshRiders(true)
    } catch (e) {
      toast.error('?€???¤íŒ¨: ?¤íŠ¸?Œí¬ ?¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤.')
    } finally {
      setSaving(false)
    }
  }
  ...
  const toggleStatus = async (rider: Rider) => {
    ...
    try {
      ...
      if (!res.ok) { const d = await res.json().catch(() => ({})); toast.error('?íƒœ ë³€ê²??¤íŒ¨: ' + (d?.error ?? '')); return }
      toast.success(`${rider.name} ?¼ì´?”ë? ${newStatus === 'active' ? '?œì„±?? : 'ë¹„í™œ?±í™”'}?ˆìŠµ?ˆë‹¤.`)
      refreshRiders(true)
    } catch {
      toast.error('?íƒœ ë³€ê²??¤íŒ¨: ?¤íŠ¸?Œí¬ ?¤ë¥˜')
    }
  }
  ...
  const deleteRider = async (rider: Rider) => {
    try {
      ...
      if (!res.ok) { const d = await res.json().catch(() => ({})); toast.error('?? œ ?¤íŒ¨: ' + (d?.error ?? '')); return }
      toast.success(`${rider.name} ?¼ì´?”ê? ?? œ?˜ì—ˆ?µë‹ˆ??`)
      setDeleteConfirmId(null)
      refreshRiders(true)
    } catch {
      toast.error('?? œ ?¤íŒ¨: ?¤íŠ¸?Œí¬ ?¤ë¥˜')
    }
  }
  ...
  const parseBulkExcel = useCallback((file: File) => {
    ...
    reader.onload = (e) => {
      // cellDates: true ???‘ì? ? ì§œ ?¼ë ¨ë²ˆí˜¸ë¥?JS Date ê°ì²´ë¡??ë™ ë³€??      const wb = XLSX.read(e.target?.result, { type: 'array', cellDates: true })
      ...
      // ? ì§œ ê°’ì„ YYYY-MM-DD ë¬¸ì?´ë¡œ ë³€??      const toDateStr = (val: unknown): string => {
        if (!val) return ''
        ...
        // ?«ì ?¼ë ¨ë²ˆí˜¸ê°€ ?˜ì–´??ê²½ìš° xlsxë¡??¬ë???        if (/^\d{5}$/.test(str)) {
        ...
        // ?´ë? ? ì§œ ?•ì‹ ë¬¸ì??(YYYY-MM-DD, YYYY/MM/DD ??
      ...
      const parsed: BulkRiderRow[] = raw
        .map((row) => {
          ...
          const join_date      = toDateStr(getRaw(['ê°€?…ì¼', 'joindate', 'join']))
          const name           = get(['?¼ì´?”ëª…', '?´ë¦„', '?±ëª…', 'ê¸°ì‚¬ëª?, 'name'])
          const rider_username = get(['?„ì´??, '?¼ì´?”ì•„?´ë””', 'userid', 'username', 'id'])
          const id_number      = get(['ì£¼ë??±ë¡ë²ˆí˜¸', 'ì£¼ë?ë²ˆí˜¸', 'ì£¼ë?', 'idnumber'])
          const phone          = get(['?°ë½ì²?, '?„í™”', '?´ë???, '?¸ë“œ??, 'phone'])
          const bank_name      = get(['?€?‰ëª…', '?€??, 'bank'])
          const bank_account   = get(['ê³„ì¢Œë²ˆí˜¸', 'ê³„ì¢Œ', 'account'])
          const account_holder = get(['?ˆê¸ˆì£¼ëª…', '?ˆê¸ˆì£?, 'holder'])

          const baseError = !name ? '?¼ì´?”ëª… ?„ìˆ˜' : ''
  ...
  const handleBulkSave = async () => {
    ...
    if (validRows.length === 0) { toast.error('?±ë¡ ê°€?¥í•œ ?°ì´?°ê? ?†ìŠµ?ˆë‹¤.'); return }
    setBulkSaving(true)
    ...
    try {
      ...
      if (!res.ok) {
        toast.error('?€???¤íŒ¨: ' + (data?.error ?? res.statusText))
        return
      }
      toast.success(`${validRows.length}ëª…ì˜ ?¼ì´?”ê? ?±ë¡?˜ì—ˆ?µë‹ˆ??`)
      setBulkDialogOpen(false)
      setBulkRows([])
      setBulkFileName('')
      refreshRiders(true)
    } catch (e) {
      toast.error('?€???¤íŒ¨: ?¤íŠ¸?Œí¬ ?¤ë¥˜')
    } finally {
      setBulkSaving(false)
    }
  }
  ...
  const handleBulkDeactivate = async () => {
    ...
    toast.success(`${targets.length}ëª…ì„ ë¹„í™œ?±í™”?ˆìŠµ?ˆë‹¤.`)
  ...
  const handleBulkDelete = async () => {
    ...
    if (failCount > 0) toast.error(`${failCount}ëª??? œ ?¤íŒ¨`)
    else toast.success(`${ids.length}ëª…ì„ ?? œ?ˆìŠµ?ˆë‹¤.`)
    setSelectedIds(new Set())
    setBulkActionConfirm(null)
    setBulkProcessing(false)
    refreshRiders()
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-2xl font-bold text-white">?¼ì´??ê´€ë¦?/h2>
          <p className="text-slate-400 text-sm mt-1">?¼ì´???±ë¡, ?˜ì •, ê´€ë¦?/p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button
            onClick={() => downloadRiderList(filtered, activeTab === 'all' ? '?„ì²´' : activeTab === 'active' ? '?œì„±' : 'ë¹„í™œ??)}
            variant="outline"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        >
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-slate-400 text-xs">?œì„± ?¼ì´??/p>
        ...
        >
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-slate-400 text-xs">ë¹„í™œ???¼ì´??/p>
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-white text-base">
              {activeTab === 'all' ? '?„ì²´' : activeTab === 'active' ? '?œì„±' : 'ë¹„í™œ??} ?¼ì´??ëª©ë¡ ({sorted.length}ëª?
            </CardTitle>
            {someSelected && (
              <div className="flex items-center gap-2">
                <span className="text-slate-400 text-sm">{selectedIds.size}ëª?? íƒ??/span>
                <Button
                  size="sm"
                  variant="outline"
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-slate-700 hover:bg-transparent">
                  ...
                  <TableHead className="text-slate-400">ê°€?…ì¼</TableHead>
                  <TableHead className="text-slate-400">?¼ì´?”ëª…</TableHead>
                  <TableHead className="text-slate-400">?„ì´??/TableHead>
                  <TableHead className="text-slate-400">ì£¼ë?ë²ˆí˜¸</TableHead>
                  <TableHead className="text-slate-400">?°ë½ì²?/TableHead>
                  <TableHead className="text-slate-400">?€??/TableHead>
                  <TableHead className="text-slate-400">ê³„ì¢Œë²ˆí˜¸</TableHead>
                  <TableHead className="text-slate-400">?ˆê¸ˆì£?/TableHead>
                  <TableHead className="text-slate-400">?íƒœ</TableHead>
              ...
              <TableBody>
                {loading ? (
                  <TableRow>
                    <TableCell colSpan={11} className="text-center text-slate-500 py-8">ë¡œë”© ì¤?..</TableCell>
                  </TableRow>
                ) : paged.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={11} className="text-center text-slate-500 py-8">?±ë¡???¼ì´?”ê? ?†ìŠµ?ˆë‹¤.</TableCell>
                  </TableRow>
                ) : (
                  paged.map(rider => (
                    <TableRow
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-4 py-3 border-t border-slate-700">
              <p className="text-slate-400 text-sm">
                {(safePage - 1) * PAGE_SIZE + 1}??Math.min(safePage * PAGE_SIZE, sorted.length)} / {sorted.length}ëª?              </p>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="bg-slate-900 border-slate-700 text-white max-w-lg">
          <DialogHeader>
            <DialogTitle className="text-white">{editingRider ? '?¼ì´???˜ì •' : '?¼ì´???±ë¡'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3 py-2">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label className="text-slate-300">ê°€?…ì¼</Label>
                <Input
                  type="date"
              ...
              <div className="space-y-1.5">
                <Label className="text-slate-300">?¼ì´?”ëª… <span className="text-red-400">*</span></Label>
                <Input
                  value={form.name}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label className="text-slate-300">?„ì´??/Label>
                <Input
              ...
              <div className="space-y-1.5">
                <Label className="text-slate-300">ì£¼ë??±ë¡ë²ˆí˜¸</Label>
                <Input
            ...
            <div className="space-y-1.5">
              <Label className="text-slate-300">?°ë½ì²?/Label>
              <Input
                value={form.phone}
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="space-y-1.5">
                <Label className="text-slate-300">?€?‰ëª…</Label>
                <Input
              ...
              <div className="space-y-1.5">
                <Label className="text-slate-300">ê³„ì¢Œë²ˆí˜¸</Label>
                <Input
              ...
              <div className="space-y-1.5">
                <Label className="text-slate-300">?ˆê¸ˆì£¼ëª…</Label>
                <Input
            ...
            {editingRider && (
              <div className="space-y-1.5">
                <Label className="text-slate-300">?íƒœ</Label>
                <Select value={form.status} onValueChange={(v: 'active' | 'inactive') => setForm(f => ({ ...f, status: v }))}>
                  <SelectTrigger className="bg-slate-800 border-slate-600 text-white">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="bg-slate-800 border-slate-600">
                    <SelectItem value="active" className="text-white">?œì„±</SelectItem>
                    <SelectItem value="inactive" className="text-white">ë¹„í™œ??/SelectItem>
                  </SelectContent>
                </Select>
              </div>
            )}
          ...
          <DialogFooter>
            <Button variant="ghost" onClick={() => setDialogOpen(false)} className="text-slate-400 hover:text-white">ì·¨ì†Œ</Button>
            <Button onClick={handleSave} disabled={saving} className="bg-blue-600 hover:bg-blue-700">
              {saving ? '?€??ì¤?..' : editingRider ? '?˜ì • ?€?? : '?±ë¡'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!deleteConfirmId} onOpenChange={(open) => { if (!open) setDeleteConfirmId(null) }}>
        <DialogContent className="bg-slate-900 border-slate-700 text-white max-w-sm">
              ...
              return (
                <div className="space-y-3">
                  <p className="text-slate-300 text-sm">
                    <span className="text-white font-semibold">{target?.name}</span> ?¼ì´?”ë? ?„ì „???? œ?©ë‹ˆ??
                  </p>
          ...
          <DialogFooter>
            <Button variant="ghost" onClick={() => setDeleteConfirmId(null)} className="text-slate-400 hover:text-white">ì·¨ì†Œ</Button>
            <Button
              onClick={() => { const r = riders.find(x => x.id === deleteConfirmId); if (r) deleteRider(r) }}
              className="bg-rose-600 hover:bg-rose-700 text-white"
            >
              <Trash2 className="h-4 w-4 mr-2" />?? œ
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      {/* ?¼ê´„ ë¹„í™œ?±í™”/?? œ ?•ì¸ ?¤ì´?¼ë¡œê·?*/}
      <Dialog open={bulkActionConfirm !== null} onOpenChange={open => { if (!open) setBulkActionConfirm(null) }}>
        <DialogContent className="bg-slate-900 border-slate-700 text-white max-w-md">
          <DialogHeader>
            <DialogTitle className="text-white">
              {bulkActionConfirm === 'deactivate' ? '? íƒ ?¼ì´??ë¹„í™œ?±í™”' : '? íƒ ?¼ì´???? œ'}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            {bulkActionConfirm === 'deactivate' ? (
              <>
                <p className="text-slate-300 text-sm">
                  ? íƒ??<span className="text-white font-semibold">{filtered.filter(r => selectedIds.has(r.id) && r.status === 'active').length}ëª?/span>???œì„± ?¼ì´?”ë? ë¹„í™œ?±í™”?©ë‹ˆ??
                </p>
              ...
              <>
                <p className="text-slate-300 text-sm">
                  ? íƒ??<span className="text-white font-semibold">{filtered.filter(r => selectedIds.has(r.id)).length}ëª?/span>???¼ì´?”ë? ?„ì „???? œ?©ë‹ˆ??
                </p>
                <div className="bg-rose-900/20 border border-rose-800 rounded-lg p-3">
                  <p className="text-rose-300 text-xs flex items-center gap-1.5">
                    <AlertTriangle className="h-3.5 w-3.5 shrink-0" />
                    ?? œ???°ì´?°ëŠ” ë³µêµ¬?????†ìŠµ?ˆë‹¤. ê´€???•ì‚° ?°ì´?°ë„ ?¨ê»˜ ?? œ?©ë‹ˆ??
                  </p>
                </div>
              </>
            )}
          ...
          <DialogFooter>
            <Button variant="ghost" onClick={() => setBulkActionConfirm(null)} className="text-slate-400 hover:text-white" disabled={bulkProcessing}>ì·¨ì†Œ</Button>
            <Button
            ...
            >
              {bulkProcessing ? 'ì²˜ë¦¬ ì¤?..' : bulkActionConfirm === 'deactivate' ? (
                <><UserX className="h-4 w-4 mr-2" />ë¹„í™œ?±í™”</>
              ) : (
                <><Trash2 className="h-4 w-4 mr-2" />?? œ</>
              )}
            </Button>
      ...
      <Dialog open={bulkDialogOpen} onOpenChange={(open) => { setBulkDialogOpen(open); if (!open) { setBulkRows([]); setBulkFileName('') } }}>
        <DialogContent className="bg-slate-900 border-slate-700 text-white max-w-5xl max-h-[90vh] flex flex-col">
          ...
          <div className="flex-1 overflow-auto space-y-4 py-2">
            <div className="flex items-center justify-between bg-slate-800/60 rounded-lg p-3">
              <div className="flex items-center gap-2 text-sm text-slate-300">
                ...
                <span>
                  <span className="font-bold text-white">?¼ì´?”ëª…</span>?€ ?„ìˆ˜?…ë‹ˆ??
                  ì»¬ëŸ¼ ?œì„œê°€ ?¬ë¼???¤ë”ëª…ìœ¼ë¡??ë™ ?¸ì‹?©ë‹ˆ??
                </span>
              </div>
            ...
            {bulkRows.length === 0 ? (
              ...
              >
                ...
                <p className="text-white font-medium mb-1">?‘ì? ?Œì¼???œë˜ê·¸í•˜ê±°ë‚˜ ?´ë¦­?˜ì—¬ ?…ë¡œ??/p>
                <p className="text-slate-400 text-sm">ì§€???•ì‹: .xlsx, .xls, .csv</p>
              </div>
            ) : (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <Badge className="bg-emerald-800 text-emerald-300">
                      <CheckCircle className="h-3 w-3 mr-1" />? íš¨ {bulkRows.filter(r => r.valid).length}ëª?                    </Badge>
                    {bulkRows.filter(r => !r.valid).length > 0 && (
                      <Badge className="bg-rose-800 text-rose-300">
                        <AlertTriangle className="h-3 w-3 mr-1" />?¤ë¥˜ {bulkRows.filter(r => !r.valid).length}??                      </Badge>
                    )}
                  </div>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="border-slate-700 hover:bg-transparent">
                        ...
                        <TableHead className="text-slate-400">?¼ì´?”ëª…</TableHead>
                        <TableHead className="text-slate-400">?„ì´??/TableHead>
                        <TableHead className="text-slate-400">ì£¼ë??±ë¡ë²ˆí˜¸</TableHead>
                        <TableHead className="text-slate-400">?°ë½ì²?/TableHead>
                        <TableHead className="text-slate-400">?€?‰ëª…</TableHead>
                        <TableHead className="text-slate-400">ê³„ì¢Œë²ˆí˜¸</TableHead>
                        <TableHead className="text-slate-400">?ˆê¸ˆì£¼ëª…</TableHead>
                        <TableHead className="text-slate-400">?íƒœ</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {bulkRows.map((row, i) => (
                        <TableRow key={i} className={`border-slate-700 ${!row.valid ? 'bg-rose-900/10' : 'hover:bg-slate-800/50'}`}>
                          ...
                          <TableCell className={`font-medium ${row.valid ? 'text-white' : 'text-rose-400'}`}>
                            {row.name || <span className="text-slate-500 italic">ë¹„ì–´?ˆìŒ</span>}
                          </TableCell>
                          ...
                          <TableCell>
                            {row.valid
                              ? <Badge className="bg-emerald-900/40 text-emerald-300 text-xs">?•ìƒ</Badge>
          ...
          <DialogFooter className="border-t border-slate-700 pt-4">
            <Button variant="ghost" onClick={() => setBulkDialogOpen(false)} className="text-slate-400 hover:text-white">ì·¨ì†Œ</Button>
            {bulkRows.length > 0 && (
              <Button onClick={handleBulkSave} disabled={bulkSaving || bulkRows.filter(r => r.valid).length === 0}
                className="bg-emerald-600 hover:bg-emerald-700">
                {bulkSaving
                  ? <><Loader2 className="h-4 w-4 animate-spin mr-2" />?€??ì¤?..</>
                  : <><CheckCircle className="h-4 w-4 mr-2" />{bulkRows.filter(r => r.valid).length}ëª??±ë¡</>}
              </Button>
            )}
          </DialogFooter>
        </DialogContent>
